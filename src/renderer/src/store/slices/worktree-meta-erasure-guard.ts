/**
 * Guards required Worktree fields against erasure by a metadata update.
 *
 * A present-but-undefined key in a spread ERASES the field. That is the intended
 * wire signal for clearing optional metadata, but on a field `Worktree` declares
 * required it produced a live `displayName: undefined` that crashed the worktree
 * palette (crash a1f81ea1).
 */
