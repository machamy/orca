...
export type RepoUpdate = Partial<
  Pick<
    ...
    | 'projectGroupId'
    | 'projectGroupOrder'
    // Fork: Unity worktree colour settings and worktree folders. Adding a Repo
    // field means adding it here AND to the RPC zod schema, which strips
    // unlisted keys at runtime — see docs/worktree-folders-design.md §5.
    | 'unityAutoSeedCache'
    | 'unityWorktreeTint'
    | 'unityTintInSidebar'
    | 'unityTintOverrides'
