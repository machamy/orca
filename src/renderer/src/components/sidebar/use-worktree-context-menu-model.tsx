...
import { useWorktreeContextMenuLifecycleComplete } from './use-worktree-context-menu-lifecycle-complete'
...
export type WorktreeContextMenuProps = {
  ...
  /** Fork: the sidebar's entry point into the default-worktree switch dialog. */
...
export function useWorktreeContextMenuModel({
  ...
  const unityMenu = useUnityWorktreeMenu({ worktree, repo, menuOpen, isDeleting, allWorktrees })
  ...
  // `dialogPending`: the fork's unity confirm dialog (and an in-flight seed)
  // outlive the menu; the Agent Map wrapper unmounting here destroyed it mid-question.
  useWorktreeContextMenuLifecycleComplete({
    onLifecycleComplete,
    menuOpen,
    ...
    lifecycleStartedRef,
    dialogPending: unityMenu.lifecyclePending
  })
