...
> **전환 전에 커밋하거나 stash 하세요.** 이 포크에서 가장 침습적인 동작입니다 — 양쪽
> 워크트리의 체크아웃을 다시 쓰고 커밋 안 된 작업을 그 사이로 옮기며, *에이전트가 브랜치를
> 따라가기*를 켜면 두 워크스페이스의 세션 메타데이터까지 맞바꿉니다. 원본은 이 경로를 전혀
> 돌지 않아서 포크 자체 테스트에만 기대고 있습니다. 도중에 끊기면(오르카 종료, 머신 이상,
> 중간 실패) 체크아웃이나 에이전트 세션이 예상 못 한 곳에 남을 수 있습니다. 깨끗한
> 워크트리끼리는 안전하게 바뀝니다 — 커밋 안 된 작업이 쌓여 있을 때가 한 번 더 생각할
> 상황입니다.
